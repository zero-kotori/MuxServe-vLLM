...
void reshape_and_cache(
  ...
  torch::Tensor& value_cache,   // [num_blocks, num_heads, head_size, block_size],  [num_blocksxnum_heads, head_size/x, block_size, x]
...
template<typename scalar_t>
__global__ void headwise_reshape_and_cache_kernel(
  const scalar_t* __restrict__ key,     // [num_tokens, num_heads, head_size]
  const scalar_t* __restrict__ value,   // [num_tokens, num_heads, head_size]
  scalar_t* __restrict__ key_cache,     // [num_blocks*num_heads, head_size/x, block_size, x]
  scalar_t* __restrict__ value_cache,   // [num_blocks*num_heads, head_size, block_size]
  ...
  const int key_stride,
  const int value_stride,
  const int slot_mapping_stride,
  const int num_heads,
  const int head_size,
  const int block_size,
  const int x) {
  const int token_idx = blockIdx.x;
  key += token_idx * key_stride;
  value += token_idx * value_stride;
  slot_mapping += token_idx * slot_mapping_stride;
  ...
  for (int i = threadIdx.x; i < n; i += blockDim.x) {
    ...
    const int64_t slot_idx = slot_mapping[head_idx];
    const int64_t block_idx = slot_idx / block_size;
    const int64_t block_offset = slot_idx % block_size;

    const int64_t tgt_key_idx = block_idx * head_size * block_size
                            + x_idx * block_size * x
                            + block_offset * x
                            + x_offset;
    const int64_t tgt_value_idx = block_idx * head_size * block_size
                              + head_offset * block_size
                              + block_offset;
    key_cache[tgt_key_idx] = __ldg(&key[i]);
    value_cache[tgt_value_idx] = __ldg(&value[i]);
  }
}

} // namespace vllm

void headwise_reshape_and_cache(
  torch::Tensor& key,           // [num_tokens, num_heads, head_size]
  torch::Tensor& value,         // [num_tokens, num_heads, head_size]
  torch::Tensor& key_cache,     // [num_blocks*num_heads, head_size/x, block_size, x]
  torch::Tensor& value_cache,   // [num_blocks*num_heads, head_size/x, block_size, x]
  torch::Tensor& slot_mapping)  // [num_tokens, num_heads]
{
  ...
  int key_stride = key.stride(0);
  int value_stride = value.stride(0);
  int slot_mapping_stride = slot_mapping.stride(0);
  ...
  VLLM_DISPATCH_FLOATING_TYPES(
    ...
    "reshape_and_cache_kernel",
    [&] {
      vllm::headwise_reshape_and_cache_kernel<scalar_t><<<grid, block, 0, stream>>>(
        key.data_ptr<scalar_t>(),
        value.data_ptr<scalar_t>(),
        key_cache.data_ptr<scalar_t>(),
        value_cache.data_ptr<scalar_t>(),
